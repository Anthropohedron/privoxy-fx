...
function TagMatcher(pattern) {
  ...
  if (exactHeaderRE.test(frags[2])) {
    this.exactHeader = frags[2].substring(1).toLowerCase();
    this.valueRE = frags[3] && new RegExp(frags[3]);
  } else {
    this.valueRE = new RegExp(frags[2] + ":\\s*" + frags[3]);
  }
}
